...
case 'igstalk':
case 'stalkig':
if (isLimit(sender, limitawal, limit)) return sendButMessage(from, lang.limit(prefix), `© ${ownername}`, [{buttonId: 'limit', buttonText: {displayText: `Check Limit`, },type: 1,}]);

            if (args.length === 1)  return alpha.reply(from, 'Kirim perintah *!igstalk username*\nConntoh *!igstalk zeusxz_store*', id)
            const stalk = await get.get(`https://cakrayp.herokuapp.com/api/instagram/stalk/?username=${args[1]}&apikey=cakrayp24Q6`).json()
            if (stalk.error) return alpha.reply(from, stalk.error, id)            
            const caps = `➸ *Full Nama* : ${stalk.data.result.fullname}\n➸ *Username* : ${stalk.data.result.username}\n➸ *Jumlah Followers* : ${stalk.data.result.followers}\n➸ *Jumlah Following* : ${stalk.data.result.following}\n➸ *ID* : ${stalk.data.result.id}\n➸ *Biodata* : ${stalk.data.result.biography}`
